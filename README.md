...
- **"NanookTier is not abstract and does not override abstract method
  getLevel()" / "cannot find symbol: INCORRECT_FOR_NETHERITE_TOOL"**:
  ya corregido en este repo. Eran dos cosas de `NanookTier.java`
  especificas de 1.20.1: la interfaz `Tier` todavia exige implementar
  `getLevel()` (se elimino recien en versiones posteriores), y el tag
  `INCORRECT_FOR_NETHERITE_TOOL` no existe hasta 1.20.5+, asi que en
  1.20.1 hay que usar `INCORRECT_FOR_DIAMOND_TOOL` (netherite no bloquea
  ningun bloque adicional respecto a diamante en vanilla).
- **"Found Gradle version Gradle 7.x. Minimum required is Gradle 8.1"**:
  ya corregido en este repo (el workflow ahora instala Gradle 8.1.1,
  que es lo que pide ForgeGradle 6.x). Si lo ves de nuevo, revisa que
  `gradle-version` en `.github/workflows/build.yml` diga `8.1.1`.
